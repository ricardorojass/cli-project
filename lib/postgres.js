...
async function createFakeSecrets () {
  for (let secret = 0; secret <= 2000; secret++) {
    const username = `ricardo-${secret}`
    const secretName = 'secretName'
    const secretValue = 'secret value'

    await client.query('INSERT INTO secrets (username, name, value) VALUES ($1, $2, $3)', [username, secretName, secretValue])
  }
  await client.end()
}
